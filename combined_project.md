...
def report_proforma_invoice(procurement_id: int):
    procurement = (
        Procurement.query.options(
            ...
            joinedload(Procurement.handler_assignment).joinedload("department"),
            joinedload(Procurement.handler_assignment).joinedload("directory"),
...
class PersonnelDepartmentAssignment(db.Model):
    ...
    @property
    def display_name(self) -> str:
        person = self.personnel.display_selected_label() if self.personnel else "—"
...
IMPORTANT CHANGE
----------------
Organization membership is now assignment-based.
...
- Procurement handler dropdown can later use these assignment rows directly.
...
IMPORTANT DOMAIN CHANGE
-----------------------
Directory/Department assignment must no longer be edited from the Personnel
form page.

Organizational placement is now managed centrally only from:
    /admin/organization-setup

Therefore:
- create/edit personnel no longer accept directory_id / department_id as form
  inputs
- those fields are forced to None here
- the Personnel form page becomes person-centric, not placement-centric
"""
...
def execute_edit_personnel(
    ...
    *,
    ...
    # Canonical placement is managed only in organization setup.
    person.directory_id = None
...
IMPORTANT CHANGE
----------------
Handler selection is now assignment-based, not just person-based.
...
def execute_create_procurement(
    ...
    *,
    ...
    ----------------------
    `handler_personnel_id` from the form now contains the selected
    PersonnelDepartmentAssignment id.
    ...
    handler_assignment_id = parse_optional_int(form_data.get("handler_personnel_id"))
...
IMPORTANT CHANGE
----------------
Handler selection is now assignment-based.
...
def execute_edit_procurement(
    ...
    *,
    ...
    IMPORTANT HANDLER RULE
    ----------------------
    The submitted `handler_personnel_id` field now carries the selected
    assignment id.
          ...
          <div class="card bg-light border-0 h-100">
            <div class="card-body py-3 small">
              ...
              <ul class="mb-0">
                ...
                <li>Διεύθυνση και τηλέφωνο χρειάζονται στα reports.</li>
              </ul>
            </div>
          </div>
        </div>
