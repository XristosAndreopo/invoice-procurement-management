...
FILE: .\app\reports\common\__init__.py
```python
"""
Shared utilities for DOCX report generation.

This package centralizes:
- generic formatting helpers
- Greek amount/date helpers
- common DOCX placeholder / table utilities
- report-domain resolution helpers

The goal is to keep individual report modules focused on:
- template-specific mappings
- template-specific table rendering
- template-specific business wording
"""

```

FILE: .\app\reports\common\amounts.py
```python
"""
...
FILE: .\app\reports\common\docx_utils.py
```python
"""
...
FILE: .\app\reports\common\domain.py
```python
"""
...
FILE: .\app\reports\common\formatting.py
```python
"""
...
FILE: .\app\reports\contract_docx.py
```python
"""
...
FILE: .\app\reports\instrumentation.py
```python
"""
...
<div class="d-flex align-items-center justify-content-between mb-3">
  ...
  <div class="d-flex gap-2 flex-wrap justify-content-end">
    ...
    {% if show_all_reports %}
      ...
      <a
        class="btn btn-sm btn-outline-primary"
        href="{{ url_for('procurements.report_contract_docx', procurement_id=procurement.id) }}"
        title="Λήψη Σύμβασης (DOCX)"
      >
        Σύμβαση
      </a>
...
{% set current_page = pager.get("page", 1) %}
{% set current_per_page = pager.get("per_page", 25) %}
{% set total_items = pager.get("total_items", 0) %}
{% set total_pages = pager.get("total_pages", 1) %}
{% set page_numbers = pager.get("page_numbers", []) %}
{% set start_index = pager.get("start_index", 0) %}
{% set end_index = pager.get("end_index", 0) %}
...
<div class="card glass-card shadow-sm border-0">
  <div class="card-body">
    <form id="filters-form" method="get" action="{{ request.path }}" class="mb-3">
      <input type="hidden" name="page" value="1">

      <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-2 mb-3">
        <div class="small text-muted">
          {% if total_items > 0 %}
            Εμφανίζονται εγγραφές {{ start_index }}–{{ end_index }} από {{ total_items }}.
          {% else %}
        ...
        <div class="d-flex align-items-center gap-2">
          <label for="per_page" class="small text-muted mb-0">Ανά σελίδα</label>
          <select id="per_page" name="per_page" class="form-select form-select-sm js-filter text-center" style="width:auto;">
            {% for size in (pager.get("per_page_options", [25, 50, 100])) %}
    ...
    {% if total_pages > 1 %}
      <form method="get" action="{{ request.path }}" class="mt-3">
        ...
        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-2">
          <div class="small text-muted">
            Σελίδα {{ current_page }} από {{ total_pages }}
          </div>
          ...
          <div class="d-flex flex-wrap align-items-center gap-2">
            <button type="submit"
                    ...
                    value="1"
                    class="btn btn-sm btn-outline-secondary"
                    {% if not pager.get("has_prev") %}disabled{% endif %}>
              « Πρώτη
            </button>

            <button type="submit"
                    name="page"
                    value="{{ pager.get('prev_page', 1) }}"
                    class="btn btn-sm btn-outline-secondary"
                    {% if not pager.get("has_prev") %}disabled{% endif %}>
              ‹ Προηγούμενη
            </button>
            ...
            {% for n in page_numbers %}
              <button type="submit"
                      name="page"
                      value="{{ n }}"
                      class="btn btn-sm {% if n == current_page %}btn-primary{% else %}btn-outline-secondary{% endif %}">
                {{ n }}
              </button>
            {% endfor %}
            ...
            <button type="submit"
                    name="page"
                    value="{{ pager.get('next_page', total_pages) }}"
                    class="btn btn-sm btn-outline-secondary"
                    {% if not pager.get("has_next") %}disabled{% endif %}>
              Επόμενη ›
            </button>
