...
def execute_create_service_unit(form_data: Mapping[str, Any]) -> OperationResult:
    ...
    unit = ServiceUnit(
        ...
        deputy_manager_personnel_id=None,
    )
...
def execute_assign_service_unit_roles(
    ...
    """
    Validate and assign manager/deputy roles for a ServiceUnit.

    BUSINESS RULES
    --------------
    - manager and deputy are optional
    - if provided, they must refer to active Personnel
    - the same person cannot be both manager and deputy
    - selected personnel must be unique across ServiceUnits for the same role,
      according to the existing project behavior
    """
    before = serialize_model(unit)

    manager_personnel_id_raw = form_data.get("manager_personnel_id")
    deputy_personnel_id_raw = form_data.get("deputy_manager_personnel_id")
    ...
    if deputy_personnel_id_raw:
        try:
            ...
            return OperationResult(
                ok=False,
                flashes=(FlashMessage("Μη έγκυρος Deputy Manager.", "danger"),),
            )
    ...
    if (
        ...
        return OperationResult(
            ok=False,
            flashes=(FlashMessage("Ο ίδιος/η ίδια δεν μπορεί να είναι και Manager και Deputy.", "danger"),),
        )
    ...
    if deputy_personnel_id is not None:
        ...
        deputy_used_elsewhere = ServiceUnit.query.filter(
            ServiceUnit.deputy_manager_personnel_id == deputy_personnel_id,
            ServiceUnit.id != unit.id,
        ).first()
    ...
    unit.deputy_manager_personnel_id = deputy_personnel_id
...
def execute_delete_service_unit(unit: ServiceUnit) -> OperationResult:
    ...
    unit.deputy_manager_personnel_id = None
    db.session.flush()
